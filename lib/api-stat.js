		...
		if (path.length < 1) {
			...
			urls = urls
				.sort(function(pre, nxt){
					var pre_stat = status[pre];
					var nxt_stat = status[nxt];
					if (sort === 'len') {
						return pre_stat.total_out_len > nxt.total_out_len;
					}else{
						return pre_stat.call_num > nxt.call_num
					}
				})
				.slice(0, limit)
				.map(function (url) {
